        ...
        if fname.endswith(".html"):
            ...
            try: 
              ...
              if exists(outpath):
                  ...
                  # outpath = join(OUTPUT_DIR, dirpath.replace(SCRAPE_DIR, ''))
              print(outpath)
              with open(outpath, 'w') as outfile:
                  # print(f"<!DOCTYPE html><html><body>", file=outfile)
                  # print(f"<h1>title</h1>", file=outfile)
                  # print("\n", file=outfile)
                  # print(content, file=outfile)
                  # print(f"</body></html>", file=outfile)
